...
const fetchMultiCurve = async () => {
    try {
        ...
        const resp = await fetch(`http://localhost:8000/yields/multi-history?bond_type=${props.activeTab}&limit=6`)
...
const fetchHistory = async () => {
    try {
        const resp = await fetch(`http://localhost:8000/history-yields?bond_type=${props.activeTab}&term=${selectedTerm.value}`)
